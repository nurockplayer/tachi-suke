...
describe("TachiSuke project scaffold", () => {
  ...
  it("includes static SEO discovery and social metadata hooks", () => {
    ...
    for (const sectionPath of ["/articles", "/mobile", "/tools"]) {
      assert.match(
        notFoundPage,
        new RegExp(`localizePath\\(locale,\\s*"${sectionPath}"\\)`),
        `custom 404 page should link locale recovery cards to ${sectionPath}`
      );
    }
  ...
  it("includes Phase 1B content depth for articles, mobile plans, and area guides", () => {
    ...
    assert.ok(articles.some((article) => article.slug === "japan-family-restaurants-dennys-gusto-royal-host"), "zh-tw family restaurant article should exist");
    ...
    assert.ok(articles.some((article) => article.slug === "japan-emergency-disaster-basics-zh-tw"), "zh-tw emergency/disaster article should exist");

...
- Built global and locale RSS feeds must keep reviewed channel metadata, atom self links, parseable dates, exact public article item URL coverage, permalink-backed GUIDs, supported item languages, and no utility/account/search-index routes.
...
- `pnpm check:seo` should verify every sitemap-derived indexable public HTML page has supported, non-duplicated `hreflang` head links that point only at public sitemap HTML pages.

...
This document records the current MVP state after Phase 1AW ward-office moving-in procedure article work. It should not be read as a promise that auth, database, database-backed submissions, support storage, saved checklist state, or favorites already work.
...
- Thirty-one public article pages across `zh-tw`, `en`, `ja`, and `ko`, including Phase 1B decision-oriented content, Phase 1AG/1AP commuter pass articles, Phase 1AH/1AP residence administration articles, Phase 1AR rental initial-cost articles, Phase 1AV apartment-viewing practical Japanese articles, and Phase 1AW ward-office moving-in procedure articles.

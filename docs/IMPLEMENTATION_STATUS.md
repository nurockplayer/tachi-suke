...
This document records the current MVP state after Phase 1DP RSS channel metadata guard checks. It should not be read as a promise that auth, database, database-backed submissions, support storage, saved checklist state, or favorites already work.
...
- RSS output checks verify channel metadata, atom self links, parseable feed/item dates, item URL coverage, permalink-backed GUIDs, supported item languages, and utility-route exclusion.
...
- Web manifest output uses the reviewed TachiSuke app identity, root start/scope values, same-site SVG icon, standalone display mode, and brand colors.
